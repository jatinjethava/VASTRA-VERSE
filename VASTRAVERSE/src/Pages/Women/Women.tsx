...
export const Women = () => {
    ...
    const { data, isLoading, isError } = useShopBySlug("women", filter.category, filter.fit, filter.maxPrice, filter.isFeatured, filter.isBestSeller, filter.isNewArrival, filter.limitedEdition);
    const { data: parentCategory } = useShopBySlug("women");
    const { data: categoriesData } = useGetChildCategoriesBySlug("women");
    ...
    return (
        <div className="min-h-screen py-6 sm:py-8 md:py-10 lg:py-12 px-4 sm:px-6 lg:px-8 relative">
            ...
            <div className="max-w-7xl mx-auto text-center mb-6 sm:mb-12 space-y-3 sm:space-y-4">
                ...
                <h1 className="text-xl sm:text-3xl md:text-4xl lg:text-4xl xl:text-5xl font-black text-gray-500 tracking-tight">WOMEN'S <span className="text-gray-800">STREETWEAR</span> COLLECTION</h1>
